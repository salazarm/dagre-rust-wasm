...
pub fn sort(entries: &Vec<ResolvedBaryEntry>, bias_right: &bool) -> SubgraphResult {
  let parts: PartitionResponse<ResolvedBaryEntry> = util::partition(entries, Box::new(|val: &ResolvedBaryEntry| -> bool {
    val.barycenter.is_some()
  }));

  let mut sortable = parts.lhs.clone();
  sortable.sort_by(|e1, e2| {
    compare_with_bias(e1, e2, bias_right)
  });
  let mut unsortable = parts.rhs.clone();
  unsortable.sort_by(|e1, e2| {
    e2.i.cmp(&e1.i)
  });
...
fn consume_unsortable(vs: &mut Vec<String>, unsortable: &mut Vec<ResolvedBaryEntry>, index: &mut usize) -> usize {
  if unsortable.len() == 0 {
    return index.clone();
  }
