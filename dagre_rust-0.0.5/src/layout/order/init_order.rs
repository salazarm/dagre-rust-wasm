...
pub fn init_order(g: &Graph<GraphConfig, GraphNode, GraphEdge>) -> Vec<Vec<String>> {
  ...
  let mut simple_nodes: Vec<String> = g.nodes().into_iter().filter(|v| {
    g.children(v).len() == 0
  }).collect();
  let max_rank = simple_nodes.iter().map(|v| {
    g.node(v).unwrap().rank.clone().unwrap_or(0)
  }).max().unwrap_or(0);
  let mut layers: Vec<Vec<String>> = (0..=max_rank).map(|_| -> Vec<String> {
    vec![]
  }).collect();

  fn dfs(v: &String, g: &Graph<GraphConfig, GraphNode, GraphEdge>, visited: &mut OrderedHashMap<String, bool>, layers: &mut Vec<Vec<String>>) {
