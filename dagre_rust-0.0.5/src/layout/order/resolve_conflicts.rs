...
pub fn resolve_conflicts (entries: &Vec<Barycenter>, cg: &Graph<GraphConfig, GraphNode, GraphEdge>) -> Vec<ResolvedBaryEntry> {
  ...
  let mut source_set: Vec<ResolvedBaryEntry> = mapped_entries.values().filter(|entry| {
    entry.indegree == 0
  }).cloned().collect();
...
fn do_resolve_conflicts(source_set: &mut Vec<ResolvedBaryEntry>) -> Vec<ResolvedBaryEntry> {
  ...
  fn handle_in(v_idx: &usize, u_idx: &usize, source_hash: &mut OrderedHashMap<usize, ResolvedBaryEntry>) {
    ...
    if u_entry.barycenter.is_none()
      ...
      || u_entry.barycenter.clone().unwrap_or(0.0) >= v_entry.barycenter.clone().unwrap_or(0.0) {
      merge_entries(v_idx, u_idx, source_hash);
    }
  }

  fn handle_out(v_idx: &usize, w_idx: &usize, source_hash: &mut OrderedHashMap<usize, ResolvedBaryEntry>, source_set: &mut Vec<ResolvedBaryEntry>) {
  ...
  return entries.into_iter().filter(|entry| {
    !entry.merged.clone().unwrap_or(false)
  }).collect();
}

fn merge_entries(target_idx: &usize, source_idx: &usize, source_hash: &mut OrderedHashMap<usize, ResolvedBaryEntry>) {
  let mut sum = 0.0;
  let mut weight = 0.0;
