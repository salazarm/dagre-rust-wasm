...
pub fn cross_count(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, layering: &mut Vec<Vec<String>>) -> usize {
  let mut cc = 0;
  let mut i = 1;
...
pub fn two_layer_cross_count(
  ...
  // (pos, weight)
  let south_entries: Vec<(usize, f32)> = layering.get(north_idx.clone())
    .cloned()
    .unwrap_or(vec![]).into_iter()
    .map(|v| -> Vec<(usize, f32)> {
      let mut out_edges: Vec<(usize, f32)> = g.out_edges(&v, None).unwrap_or(vec![]).into_iter().map(|e| {
        let pos = south_pos.get(&e.w).cloned().unwrap_or(0);
        let label = g.edge_with_obj(&e).cloned().unwrap();
        (pos, label.weight.unwrap_or(0.0))
      }).collect();
      out_edges.sort_by(|e1, e2| {
        e1.0.cmp(&e2.0)
      });

      out_edges
    }).collect::<Vec<Vec<(usize, f32)>>>().concat();
