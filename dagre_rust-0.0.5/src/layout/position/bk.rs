...
pub fn find_type_2_conflicts(
  ...
  fn scan(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, south: &Vec<String>, south_pos: &usize, south_end: &usize, prev_north_border: &i32, next_north_border: &i32, conflicts: &mut OrderedHashMap<String, OrderedHashMap<String, bool>>) {
        ...
        preds.iter().for_each(|u| {
          ...
          if let Some(u_node) = u_node_ {
            ...
            if u_node.dummy.is_some() && (
              &u_node_order < prev_north_border || &u_node_order > next_north_border
            ) {
  ...
  fn visit_layer(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, north: &Vec<String>, south: &Vec<String>, conflicts: &mut OrderedHashMap<String, OrderedHashMap<String, bool>>) {
        ...
        if v_node.dummy.is_some() && v_node.dummy.clone().unwrap() == "border" {
          ...
          if predecessors_.is_some() {
            ...
            if predecessors.len() > 0 {
              next_north_pos = g.node(&predecessors[0]).unwrap().order.clone().unwrap_or(0) as i32;
              scan(g, &south, &south_pos, &south.len(), &next_north_pos, &(north.len() as i32), conflicts);
...
pub fn horizontal_compaction(
  ...
  fn pass1(
    ...
    let val: f32 = in_edges.iter().fold(0.0, |acc, e| {
      let ev: f32 = xs.get(&e.v).cloned().unwrap() + (block_g.edge_with_obj(&e).cloned().unwrap_or(0.0));
      acc.max(ev)
    });
  ...
  fn pass2(
    ...
    let min: f64 = out_edges.iter().fold(f64::INFINITY, |acc, e| {
      let ev: f32 = xs.get(&e.w).cloned().unwrap() - (block_g.edge_with_obj(&e).cloned().unwrap_or(0.0));

      acc.min(ev as f64)
    });

    let node = g.node(elem).unwrap();
    if min != f64::INFINITY && node.border_type.is_some() && node.border_type.as_ref().unwrap() != border_type {
