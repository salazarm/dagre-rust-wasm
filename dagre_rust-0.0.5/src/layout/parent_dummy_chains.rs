  ...
  for v_ in dummy_chains.iter() {
    ...
    while v != edge_obj.w {
      ...
      if ascending {
        ...
        while
          path_v != lca &&
            g.node(path_v).unwrap_or(&empty_node).max_rank.clone().unwrap_or(0) < node.rank.clone().unwrap_or(0) {
          path_idx += 1;
      ...
      if !ascending {
        ...
        while path_idx < path.len() - 1 &&
          g.node(path_v).unwrap().min_rank.clone().unwrap_or(0) <= node.rank.clone().unwrap_or(0) {
          path_idx += 1;
          path_v = path.get(path_idx + 1).unwrap_or(&empty_string);
        }
      }
      ...
      v = g.successors(&v).unwrap_or(vec![]).get(0).unwrap_or(&empty_string).clone();
    }
  }
}
...
fn find_path(g: &Graph<GraphConfig, GraphNode, GraphEdge>, post_order_nums: &OrderedHashMap<String, (i32, i32)>, v: &String, w: &String) -> (Vec<String>, String) {
